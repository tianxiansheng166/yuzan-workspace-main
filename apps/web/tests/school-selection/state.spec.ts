...
describe("membership role redirects", () => {
  it("maps every supported role and fails closed by type", () => {
    ...
    expect(routeForMembershipRole("VOLUNTEER")).toBe("/training/volunteer");
    expect(routeForMembershipRole("SCHOOL_ADMIN")).toContain(
      "school-admin-pending",
    );
    expect(routeForMembershipRole("PLATFORM_ADMIN")).toBe("/studio");
  });
});
