(() => {
  ...
  const state = { attempt: null, items: [], currentIndex: 0, save: 'IDLE', error: '', timers: new Map(), media: null, blob: null, blobUrl: null, elapsed: 0 };
  ...
  function renderStimulus(spec) {
    ...
    if (s.type === 'IMAGE') {
      const url = s.url || s.imageUrl;
      return `<section class="runner-stimulus image-stimulus">${title}${instruction}${url ? `<img src="${safe(url)}" alt="${safe(s.alt || s.caption || '题目图片')}">` : `<p class="runner-error">图片资源暂不可读取。</p>`}${s.caption ? `<small>${safe(s.caption)}</small>` : ''}</section>`;
    }
    if (s.type === 'AUDIO') {
      const url = s.url || s.audioUrl || s.demoAudioUrl;
      return `<section class="runner-stimulus audio-stimulus">${title}${instruction}${url ? `<audio controls preload="metadata" src="${safe(url)}">浏览器不支持音频播放。</audio>` : `<p class="runner-error">音频资源暂不可读取。</p>`}</section>`;
    }
  ...
  function renderResponse(item, spec) {
    ...
    if (response.type === 'CHOICE') return `<div class="runner-options">${(Array.isArray(response.options) ? response.options : []).map((option, index) => {
      ...
      const image = isObject(option) ? (option.imageUrl || option.url) : '';
      return `<button class="runner-option ${String(answerValue(item)) === String(value) ? 'selected' : ''}" data-choice="${safe(value)}"><b>${safe(option?.key || String.fromCharCode(65 + index))}</b>${image ? `<img src="${safe(image)}" alt="${safe(label || '选项图片')}">` : ''}<span>${safe(label || value)}</span></button>`;
    }).join('')}</div>`;
  ...
  async function save(item, value) { state.save = 'SAVING'; render(); try { await Api.saveWrittenAnswer(attemptId, item.id, { content: { responseType: normalize(item).response.type, value }, wordCount: 0, charCount: String(value).length }); item.studentAnswer = { content: { responseType: normalize(item).response.type, value } }; state.save = 'SAVED'; } catch (error) { state.error = error.message || '网络错误'; state.save = 'ERROR'; } render(); }
  function flush() { const item = current(); const textarea = document.querySelector('[data-text]'); if (textarea && String(answerValue(item)) !== textarea.value) return save(item, textarea.value); return Promise.resolve(); }
  ...
  function bind() { document.querySelectorAll('[data-go]').forEach(button => button.onclick = async () => { await flush(); state.currentIndex = Number(button.dataset.go); render(); }); document.querySelector('[data-prev]')?.addEventListener('click', async () => { await flush(); state.currentIndex--; render(); }); document.querySelector('[data-next]')?.addEventListener('click', async () => { await flush(); state.currentIndex++; render(); }); document.querySelectorAll('[data-choice]').forEach(button => button.onclick = () => save(current(), button.dataset.choice)); const textarea = document.querySelector('[data-text]'); textarea?.addEventListener('input', () => { state.save = 'DIRTY'; clearTimeout(state.timers.get(current().id)); state.timers.set(current().id, setTimeout(() => save(current(), textarea.value), 700)); document.querySelector('[data-save-status]').textContent = statusText(); }); document.querySelector('[data-start-recording]')?.addEventListener('click', beginSpeech); document.querySelector('[data-stop-recording]')?.addEventListener('click', () => { clearInterval(state.media.clock); state.media.recorder.stop(); }); document.querySelector('[data-rerecord]')?.addEventListener('click', () => { if (state.blobUrl) URL.revokeObjectURL(state.blobUrl); state.media = null; state.blob = null; render(); }); document.querySelector('[data-upload-recording]')?.addEventListener('click', uploadSpeech); document.querySelector('[data-submit]')?.addEventListener('click', submit); }
