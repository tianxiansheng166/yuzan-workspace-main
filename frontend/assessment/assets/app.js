(() => {
  ...
  function renderProcessing(){
    ...
    const diagnosticItems = readingItems.filter(item => item.autoResult?.strategy === 'SPEECH_READING' && item.autoResult?.provider === 'local');
    const diagnosticMetric = value => typeof value === 'number' && Number.isFinite(value) ? `${Math.round(value)} /100` : '—';
    const diagnosticContent = diagnosticItems.length ? `<section class="processing-diagnostics"><article class="card"><div class="section-title"><div><h2>朗读自动学习诊断</h2><p class="muted small">自动学习诊断结果，仅供练习参考；不是正式考试分数。</p></div>${statusChip('实验性基线','gold')}</div><div class="notice" style="margin-top:14px">结果仍需教师复核，local speech scorer 尚未完成正式校准。</div><div class="item-grid" style="margin-top:14px">${diagnosticItems.map((item,index) => { const diagnostic = item.autoResult || {}; const metrics = diagnostic.metrics || {}; const tone = typeof metrics.tone === 'number' ? diagnosticMetric(metrics.tone) : '暂不可用'; return `<div class="item-card"><h4>朗读题 ${index + 1} · 参考值 ${safe(diagnostic.candidatePoints ?? '—')} / ${safe(diagnostic.maxScore ?? item.maxScore ?? '—')}</h4><p>准确度　${diagnosticMetric(metrics.accuracy)}</p><p>完整度　${diagnosticMetric(metrics.completeness)}</p><p>流利度　${diagnosticMetric(metrics.fluency)}</p><p>声调（实验）　${tone}</p><p class="muted small">状态：需要教师复核</p></div>`; }).join('')}</div></article></section>` : '';
