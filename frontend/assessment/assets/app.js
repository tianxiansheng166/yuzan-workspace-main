(() => {
  ...
  function renderRemediationResult(session, result){
    ...
    const referenceNotice = referenceReady && !completed
      ...
      : '';
    return shell(`<main class="page"><div class="hero-landscape" style="height:220px"></div><section class="report-head"><a class="muted small" href="${routes.center}">‹ 返回练习中心</a><h1 class="page-title" style="margin-top:16px">本次巩固 ${statusChip(completed ? '已完成' : referenceReady ? '参考结果已生成' : result.status || '处理中', completed || referenceReady ? 'green' : 'gold')}</h1><p class="page-subtitle">这是本次重练的完成情况，不是正式测评总分。</p></section>${referenceNotice}<section class="report-grid"><article class="card score-card"><h3 class="card-title" style="color:var(--red)">${referenceReady && !completed ? '已返回参考结果' : '本次巩固'}</h3><div class="score-number">${safe(referenceEarned)} <small style="font-size:17px;color:#777">/ ${safe(referenceMax)}</small></div>${statusChip(referencePercent, completed || referenceReady ? 'green' : 'gold')}<p class="muted small">${referenceReady && !completed ? `真实模型已返回 ${safe(result.referenceItemCount)} / ${safe(result.itemCount)} 道；其余 ${safe(result.humanReviewItemCount)} 道等待教师正式复核。` : `已完成评分 ${safe(result.completedItemCount)} / ${safe(result.itemCount)} 题；待评分 ${safe(result.pendingItemCount)} 题。`}</p></article><article class="card metric-card"><h3>专项题型</h3>${familyRows}</article></section><section class="report-bottom"><article class="card"><h2 class="card-title">题目完成情况</h2><p class="muted small">仅展示本题得分与评分状态，不展示标准答案或评分规则。</p><div class="item-grid" style="margin-top:14px">${itemRows}</div></article></section><section class="report-bottom"><article class="card"><h2 class="card-title">继续学习</h2><p class="muted">${completed ? '可以再次巩固同一批需要优先重练的题目，正式测评结果保持不变。' : referenceReady ? '参考巩固结果已可查看；教师复核后会另行保存正式分。' : '等待真实模型诊断或教师复核后会在这里显示本次巩固结果。'}</p><div style="display:flex;gap:10px;flex-wrap:wrap;margin-top:16px">${actions}</div></article></section></main>`);
  }
