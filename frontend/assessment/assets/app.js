(() => {
  ...
  const referencePreview = query.get('preview') === '1';
  ...
  function renderSpeechDiagnosticReport(session, report, diagnosticItem) {
    ...
    return `<main class="page diagnostic-report-page"><header class="report-film-head"><a href="${routes.center}" class="back-link">${icon('left')} 返回练习中心</a><div><p class="eyebrow">朗读学习档案 · 自动诊断</p><h1>这一次的声音，已经被认真保存</h1><p>自动结果仅用于学习诊断；正式成绩与结论仍由教师复核。</p></div><span class="chip gold">${state}</span></header><section class="diagnostic-report-grid"><article class="diagnostic-evidence"><div class="evidence-kicker">本次朗读文本</div><blockquote>${safe(text)}</blockquote><div class="evidence-player"><button class="play-circle" data-report-play-recording="${safe(recordingId || '')}" ${recordingId ? '' : 'disabled'}>${icon('play')}</button><div class="report-wave" aria-label="本次原始录音波形">${waveBars(64)}</div><div><strong>原始录音</strong><small>${recordingId ? '点击播放，音频仅在当前授权范围内读取。' : '录音证据正在等待保存。'}</small></div></div><div class="diagnosis-narrative"><span>本次诊断</span><p>${hasProviderResult ? '已收到本次朗读的真实维度结果，以下内容是学习诊断，不是正式分数。' : '尚未收到可展示的朗读维度结果；不会把缺失结果显示为零分。'}</p></div></article><aside class="diagnostic-summary"><div class="diagnostic-status"><span>自动诊断状态</span><strong>${state}</strong><small>建议教师复核</small></div><div class="diagnostic-overall"><span>总体结果</span><b>${overall}</b><small>${hasProviderResult ? '来自本次真实朗读诊断' : '等待真实朗读诊断'}</small></div><div class="diagnostic-dimensions">${metricCards}</div><div class="diagnostic-focus"><h2>本次重点改善</h2><ol>${focusRows}</ol></div><div class="diagnostic-next-step"><a class="btn primary" href="/student/practices/?recommendedLabel=${encodeURIComponent('独立朗读')}">${icon('arrow')} 继续朗读练习</a><p class="muted small">教师复核后将基于正式诊断生成专项巩固。</p></div><button class="btn diagnostic-evidence-toggle" data-toggle-evidence>${icon('info')} 查看评测依据</button><section class="diagnostic-evidence-drawer" data-evidence-drawer hidden><h2>评测依据</h2><p><b>基础语音评测：</b>真实朗读诊断</p><p><b>当前结果：</b>自动诊断 / 建议教师复核</p><p><b>诊断标准版本：</b>当前试运行版本</p></section></aside></section></main>`;
  }
  ...
  function renderProcessing(){
    ...
    // 真实 SpeechJob 状态映射
    const reviewedJobCount = speechJobs.filter(job => job.status === 'NEEDS_REVIEW').length;
    const failedJobCount = speechJobs.filter(job => job.status === 'FAILED').length;
    // A single unsupported oral task must not conceal diagnostics returned for
    // other recordings.  The formal session still waits for teacher review.
    const jobStatus = reviewedJobCount ? 'NEEDS_REVIEW' : failedJobCount ? 'FAILED' : speechJob?.status;
  ...
  function renderReport(){
    ...
    if (session?.purpose === 'REMEDIATION') {
      return referencePreview
        ? renderRemediationReferencePreview(session, appState.apiItems)
        : renderRemediationResult(session, appState.apiRemediationResult);
    }
    const diagnosticItem = speechDiagnosticItems(appState.apiItems)[0];
    if (!report && !diagnosticItem) {
  ...
  function renderRemediationResult(session, result){
    ...
    const families = [...(result.families || [])].sort((left, right) => (left.percentage ?? 0) - (right.percentage ?? 0));
    const familyRows = families.map(family => `<div class="remediation-family-row"><div><strong>${safe(family.displayName || family.family || '专项能力')}</strong><span>${safe(family.itemCount)} 题 · ${percentValue(family.percentage)}</span></div><b>${safe(family.earnedPoints)} / ${safe(family.maxPoints)}</b></div>`).join('') || '<p class="muted">暂无可显示的专项能力汇总。</p>';
    const referenceReady = result.referenceReady === true;
    const referenceEarned = referenceReady ? result.referenceEarnedPoints : result.earnedPoints;
    const referenceMax = referenceReady ? result.referenceMaxPoints : result.maxPoints;
    const referencePercent = referenceReady ? percentValue(result.referencePercentage) : percent;
    const actions = completed
      ...
      : `<a class="btn" href="${routes.processing}">${icon('clock')} 返回处理状态</a><button class="btn" data-retry>${icon('refresh')} 刷新结果</button>`;
    const recommendations = Array.isArray(result.recommendations) ? result.recommendations : [];
    const recommendationRows = recommendations.length
      ? recommendations.map((recommendation) => `<article class="remediation-recommendation"><div><span class="chip gold">优先提升 · ${safe(recommendation.targetFamily || '专项能力')}</span><h3>${safe(recommendation.title)}</h3><p>${safe(recommendation.reason || recommendation.guidance || recommendation.summary || '')}</p><small>${safe(recommendation.difficulty || '专项训练')} · 约 ${safe(recommendation.estimatedMinutes || '—')} 分钟</small></div><a class="btn primary" href="/student/practices/${encodeURIComponent(recommendation.practiceDefinitionId)}/">进入练习 ${icon('arrow')}</a></article>`).join('')
      : '<p class="muted">暂未找到与本次薄弱项匹配且已向你开放的下一项练习。</p>';
    const referenceNotice = referenceReady && !completed
      ? `<article class="card notice" style="margin-bottom:14px">${icon('info')} 参考巩固结果已生成：${safe(result.referenceItemCount)} 道题已有真实模型诊断；${safe(result.humanReviewItemCount)} 道题仍等待教师复核。参考分不会写入正式成绩。</article>`
      : '';
    return shell(`<main class="page remediation-report"><div class="hero-landscape" style="height:220px"></div><section class="report-head"><a class="muted small" href="${routes.center}">‹ 返回练习中心</a><h1 class="page-title" style="margin-top:16px">本次巩固 ${statusChip(completed ? '已完成' : referenceReady ? '参考结果已生成' : result.status || '处理中', completed || referenceReady ? 'green' : 'gold')}</h1><p class="page-subtitle">这是本次重练的完成情况，不计入正式测评总分。</p></section>${referenceNotice}<section class="remediation-summary-grid"><article class="card remediation-score-card"><p class="card-kicker">${referenceReady && !completed ? '已返回参考结果' : '本次巩固结果'}</p><div class="score-number">${safe(referenceEarned)} <small>/ ${safe(referenceMax)}</small></div>${statusChip(referencePercent, completed || referenceReady ? 'green' : 'gold')}<p class="muted">${referenceReady && !completed ? `已返回 ${safe(result.referenceItemCount)} / ${safe(result.itemCount)} 道诊断；其余 ${safe(result.humanReviewItemCount)} 道等待教师复核。` : `已完成 ${safe(result.completedItemCount)} / ${safe(result.itemCount)} 道巩固题。`}</p></article><article class="card remediation-family-card"><div class="section-title"><div><h2>专项能力完成情况</h2><p class="muted small">按本次真实题型汇总，不展示标准答案。</p></div></div><div class="remediation-family-list">${familyRows}</div></article></section><section class="remediation-detail-grid"><article class="card remediation-next-card"><div class="section-title"><div><p class="card-kicker">个性化下一步</p><h2>为你推荐的训练</h2><p class="muted small">根据本次得分较低的题型，从当前已开放练习中匹配。</p></div></div><div class="remediation-recommendations">${recommendationRows}</div></article><article class="card remediation-actions-card"><p class="card-kicker">继续学习</p><h2 class="card-title">下一步怎么做</h2><p class="muted">${completed ? '可选择一项推荐训练继续巩固，或再练一次同一批题目。正式测评结果保持不变。' : referenceReady ? '参考结果已可查看；教师复核后会另行保存正式分。' : '等待真实模型诊断或教师复核后会在这里显示本次巩固结果。'}</p><div class="remediation-actions">${actions}</div></article></section></main>`);
  }
  ...
  function diagnosisPanel(diagnosis){
    ...
    const domainCards = domains.map(domain => `<article class="card metric-card"><h3>${safe(domain.displayName || '能力')}</h3><div class="metric-value">${safe(domain.earnedPoints)}<small style="font-size:14px;color:#777"> / ${safe(domain.maxPoints)}</small></div><div class="metric-rail"><i style="width:${progress(domain.percentage)}%;background:var(--green)"></i><b style="left:${progress(domain.percentage)}%;background:var(--green)"></b></div><p class="muted small">${percent(domain.percentage)} · ${domain.proficiency === 'STRONG' ? '掌握较好' : domain.proficiency === 'DEVELOPING' ? '继续巩固' : '建议优先练习'}</p></article>`).join('');
    const strengthsContent = strengths.length ? strengths.map(item => `<div class="recommend-item"><div><strong>${safe(item.displayName)}</strong><p class="muted small">掌握较好 · ${safe(item.earnedPoints)} / ${safe(item.maxPoints)} · ${percent(item.percentage)}</p></div></div>`).join('') : '<p class="muted">继续保持本次认真作答的状态。</p>';
    const prioritiesContent = priorities.length ? priorities.map((item,index) => `<div class="recommend-item"><div><strong>优先提升 ${index + 1}：${safe(item.displayName)}</strong><p class="muted small">${safe(item.earnedPoints)} / ${safe(item.maxPoints)} · ${percent(item.percentage)}</p></div></div>`).join('') : '<p class="muted">这次各项能力都完成得很好，继续保持！</p>';
  ...
  function renderHistory(){
    ...
    const formalSessions = sessions.filter(session => session.purpose !== 'REMEDIATION');
    const remediationSessions = sessions.filter(session => session.purpose === 'REMEDIATION');
    const latestFormal = formalSessions[0] || null;
    const latestRemediation = remediationSessions[0] || null;
    const formatDate = value => value ? new Date(value).toLocaleDateString() : '—';
    const formalRows = formalSessions.map(session => `<article class="history-record-row"><div><span class="chip red">正式测评</span><h3>${session.type === 'READING' ? '朗读测评' : session.type === 'WRITTEN' ? '书面测评' : '综合测评'}</h3><p>${formatDate(session.completedAt)} · 朗读 ${safe(session.metrics?.reading ?? '—')} 分 · 书面 ${safe(session.metrics?.written ?? '—')} 分</p></div><div class="history-record-score"><strong>${safe(session.metrics?.overall ?? '—')}</strong><small>总分</small></div><a class="btn ghost" href="${base}/sessions/${session.sessionId}/report/">查看报告</a></article>`).join('') || '<p class="muted">还没有已完成的正式测评。</p>';
    const remediationRows = remediationSessions.map(session => { const summary = session.remediation; return `<article class="history-record-row remediation"><div><span class="chip green">专项巩固</span><h3>${safe(summary?.title || '本次巩固')}</h3><p>${formatDate(session.completedAt)} · ${summary ? `完成 ${safe(summary.earnedPoints)} / ${safe(summary.maxPoints)} 分，${percentValue(summary.percentage)}` : '结果仍在整理中'}</p></div><div class="history-record-score"><strong>${summary ? `${safe(summary.earnedPoints)} / ${safe(summary.maxPoints)}` : '—'}</strong><small>本次巩固</small></div><a class="btn primary" href="/student/practices/attempts/${encodeURIComponent(session.sessionId)}/report/">查看结果</a></article>`; }).join('') || '<p class="muted">还没有已完成的专项巩固。</p>';

    return shell(`<main class="page"><div class="hero-landscape" style="height:220px"></div><section class="hero-head" style="min-height:130px"><h1 class="page-title">学习记录</h1><p class="page-subtitle">正式测评与专项巩固分开保存，避免把练习结果当成正式成绩。</p></section><section class="history-layout"><div><div class="history-summary"><article class="card"><div class="icon red">${icon('assessment')}</div><small>正式测评</small><b style="display:block;font-size:32px">${formalSessions.length}</b><span class="muted small">${latestFormal ? `最近一次：${formatDate(latestFormal.completedAt)}` : '暂无正式测评'}</span></article><article class="card"><div class="icon">${icon('chart')}</div><small>最近正式成绩</small><b style="display:block;font-size:32px">${latestFormal?.metrics?.overall ?? '—'}</b><span class="muted small">${latestFormal?.completedAt ? formatDate(latestFormal.completedAt) : '完成正式测评后显示'}</span></article><article class="card"><div class="icon green">${icon('refresh')}</div><small>已完成巩固</small><b style="display:block;font-size:32px">${remediationSessions.length}</b><span class="muted small">${latestRemediation?.remediation ? `最近结果：${latestRemediation.remediation.earnedPoints} / ${latestRemediation.remediation.maxPoints}` : '暂无专项巩固'}</span></article></div><section class="history-record-panel card"><div class="section-title"><div><h2>正式测评</h2><p class="muted small">只记录正式测评成绩。</p></div></div>${formalRows}</section><section class="history-record-panel card"><div class="section-title"><div><h2>专项巩固</h2><p class="muted small">展示本次练习结果，不计入正式测评总分。</p></div></div>${remediationRows}</section></div><aside class="history-side"><article class="card retest"><div class="section-title"><h2>安排复测</h2><div class="icon">${icon('calendar')}</div></div><p class="muted">复测由教师在学校端发起，学生端仅可查看安排结果。</p><p class="muted small">如需复测，请联系教师。</p></article></aside></section></main>`);
