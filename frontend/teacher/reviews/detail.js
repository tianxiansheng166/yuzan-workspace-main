(() => {
  ...
  function render(detail) {
    ...
    const diagnosticTitle = isReading ? `${providerLabel(evidence.diagnostic?.provider)} · 自动诊断证据` : '本地语音诊断（仅证据，不自动给语义分）';
    const diagnosticNote = isReading ? '未校准结果只作为复核线索；正式得分必须由教师独立输入。' : '';
