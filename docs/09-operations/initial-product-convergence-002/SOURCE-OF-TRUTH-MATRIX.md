...
CP7 ruling: browser and build evidence confirms that the four entry families are routable and that auth, school selection, plans, core teacher/student reads, assignment creation/opening, submission and feedback APIs execute against the real backend. It also disproves three earlier readiness assumptions: volunteer/training/pairing repositories are unavailable and have no Prisma persistence; curriculum publication is provider-blocked; and the learning/review workflow lacks enough public transitions to complete the required loop without test-only database changes. Those routes remain `TRUTHFUL_GAP` or partial and the task is not declared READY.
