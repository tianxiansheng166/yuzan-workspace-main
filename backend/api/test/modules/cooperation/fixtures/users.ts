...
export function suspendedPrincipal(overrides: { userId?: string } = {}) {
  return {
    userId: overrides.userId ?? "suspended-1",
    roles: [MembershipRole.SCHOOL_ADMIN] as readonly MembershipRole[],
...
export function suspendedAuth(schoolId = "school-1", overrides: { userId?: string } = {}) {
  return createAuthContext("req-1", suspendedPrincipal(overrides), { schoolId });
}
